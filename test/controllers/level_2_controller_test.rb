...
class Level2Test < ActionDispatch::IntegrationTest
    ...
    setup do
        ...
        @product4 = create(:products, name: "Zapatillas", description: "Zapatillas deportivas", price: 50, brand: @brand3)
        @buyer1 = create(:buyers, name: "Buyer_1", wallet: 1000)
    end

    test "DELETE /brands/:id elimina marca existente sin productos asociados" do # 0.2 punto
        delete "/brands/#{@brand3.id}"
        assert_response :success
    end

    test "DELETE /brands/:id elimina marca y todos sus productos asociados" do # 0.7 punto
    ...
    test "DELETE /buyers/:id elimina comprador existente sin compras asociadas" do # 0.6 punto
        delete "/buyers/#{@buyer1.id}"
        assert_response :success
        assert_nil Buyer.find_by(id: @buyer1.id)
    end

    test "DELETE /products/:id elimina producto existente no comprado" do # 0.3 punto
        delete "/products/#{@product3.id}"
        assert_response :success
        assert_nil Product.find_by(id: @product3.id)
    end

    test "PATCH /products/:id actualiza producto existente" do # 0.2 punto
