...
class Level0Test < ActionDispatch::IntegrationTest
  ...
  setup do
      ...
      @buyer3 = create(:buyers, name: "Buyer_3", wallet: 3000)
  end

  test "GET /brands retorna todas las marcas existentes" do # 0.1 punto
    get "/brands"
  ...
  test "GET /brands/:id retorna marca existente" do # 0.05 punto
    get "/brands/#{@brand3.id}"
    assert_response :success
    assert_equal @brand3.name, json_response["name"]
  end

  test "GET /brands/:id lanza error 404 para marca no existente" do # 0.05 punto
    get "/brands/999"
    assert_response :not_found
  end

  test "GET /buyers retorna todos los compradores existentes" do # 0.1 punto
    get "/buyers"
    assert_response :success
  ...
  test "GET /buyers/:id retorna comprador existente" do # 0.05 punto
    get "/buyers/#{@buyer3.id}"
    assert_response :success
    assert_equal @buyer3.name, json_response["name"]
  end

  test "GET /buyers/:id lanza error 404 para comprador no existente" do # 0.05 punto
    get "/buyers/999"
    assert_response :not_found
  end

  test "POST /brands crea una nueva marca" do # 0.1 punto
  ...
  test "POST /brands no crea una marca sin nombre" do # 0.1 punto
    brand_params = { country: "Mexico" }
    post "/brands", params: { brand: brand_params }
    assert_response :unprocessable_entity
  end

  test "POST /brands no crea una marca sin país" do # 0.1 punto
    brand_params = { name: "Del Valle" }
    post "/brands", params: { brand: brand_params }
    assert_response :unprocessable_entity
  end

  test "POST /buyers crea un nuevo comprador" do # 0.1 punto
  ...
  test "POST /buyers no crea un comprador sin nombre" do # 0.1 punto
    buyer_params = { wallet: 1000000 }
    post "/buyers", params: { buyer: buyer_params }
    assert_response :unprocessable_entity
  end

  test "POST /buyers no crea un comprador sin wallet" do # 0.1 punto
